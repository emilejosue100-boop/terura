...
import { Sparkles, Flag, RefreshCw, Layers, TrendingUp, HelpCircle, Check, Search } from 'lucide-react';
...
export default function OpportunityFeed({ state, language, onStateChange }: OpportunityFeedProps) {
  ...
  const handleRefreshFeed = async () => {
    setRefreshing(true);
    try {
      const res = await fetch('/api/refresh-opportunities', { method: 'POST' });
      if (res.ok) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-text-secondary">
            {language === 'en' ? 'Web crawler scraping real Rwandan financial vehicles & micro-yield funds' : 'Ikoranabuhanga rya AI risuzuma amakuru y’amabanki n’amasoko mu Rwanda'}
          </p>
        </div>

        {/* Dynamic crawler refresh button */}
        <button
        ...
        >
          ...
          {refreshing 
            ? (language === 'en' ? 'Scanning Web...' : 'Scanning...') 
            : (language === 'en' ? 'Scrape / Refresh Opportunities' : 'Ongerera cyangwa Vugurura')}
        </button>
      </div>
            ...
            <div className="p-5 flex-1 flex flex-col justify-between">
              <div>
                ...
                <div className="flex items-center gap-1 text-[10px] text-text-secondary font-bold uppercase tracking-wider mb-4">
                  ...
                  <span className="text-primary hover:underline cursor-pointer">{opp.source}</span>
                </div>
